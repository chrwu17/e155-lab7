...
int main(void) {
  ...
  pinMode(DONE_PIN, GPIO_INPUT);   // DONE
  
  // debugging LEDs
  pinMode(SUCCESS_LED, GPIO_OUTPUT);
  pinMode(FAIL_LED, GPIO_OUTPUT);
...
void checkAnswer(char * key, char * plaintext, char * cyphertext) {
  ...
  if(correct) {
    digitalWrite(SUCCESS_LED, 1); // Success!
  } else {
    digitalWrite(FAIL_LED, 1);    // Bummer.  Test failed.
  }
  }
